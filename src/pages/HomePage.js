          ...
          <div className={`h-[500px] rounded-[2.5rem] flex flex-col border ${theme.glass} overflow-hidden`}>
            ...
            <div className="flex-1 p-6 overflow-y-auto space-y-4">
              {chatMessages.map((msg, i) => (
                <div key={i} dir="auto" className={`p-4 rounded-2xl max-w-[80%] whitespace-pre-wrap ${msg.role === 'bot' ? theme.card : 'bg-indigo-600 text-white self-end ml-auto'}`}>{msg.text}</div>
              ))}
              <div ref={chatEndRef} />
            </div>
